...
pub fn scan_all_clients(home_dir: &str, clients: &[String]) -> ScanResult {
    ...
    // Gemini CLI and Antigravity both live in ~/.gemini/tmp/ with the same file pattern.
    // Antigravity project directories contain a `.project_root` file; Gemini CLI dirs don't.
    if enabled.contains(&ClientId::Gemini) || enabled.contains(&ClientId::Antigravity) {
        let gemini_root = format!("{}/.gemini/tmp", home_dir);
        let all_session_files = scan_directory(&gemini_root, "session-*.json");
        for path in all_session_files {
            // A file's grandparent is the project dir: <root>/<project>/<chats>/session-*.json
            let is_antigravity = path
                .parent()
                .and_then(|p| p.parent())
                .map(|project_dir| project_dir.join(".project_root").exists())
                .unwrap_or(false);
            if is_antigravity {
                if enabled.contains(&ClientId::Antigravity) {
                    result.get_mut(ClientId::Antigravity).push(path);
                }
            } else if enabled.contains(&ClientId::Gemini) {
                result.get_mut(ClientId::Gemini).push(path);
            }
        }
    }
