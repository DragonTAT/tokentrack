...
pub struct GeminiMessage {
    ...
    // Accept both string and array content formats
...
pub fn parse_gemini_file(path: &Path) -> Vec<UnifiedMessage> {
    parse_for_source(path, "gemini")
}

/// Parse a Google Antigravity session file (same format as Gemini CLI)
pub fn parse_antigravity_file(path: &Path) -> Vec<UnifiedMessage> {
    parse_for_source(path, "antigravity")
}

fn parse_for_source(path: &Path, source: &str) -> Vec<UnifiedMessage> {
    let fallback_timestamp = file_modified_timestamp_ms(path);

    if path.extension().and_then(|s| s.to_str()) == Some("jsonl") {
        return parse_gemini_headless_jsonl(path, source, fallback_timestamp);
    }
    ...
    if let Ok(session) = simd_json::from_slice::<GeminiSession>(&mut bytes) {
        return parse_gemini_session(session, source, fallback_timestamp);
    }
    ...
    parse_gemini_headless_jsonl(path, source, fallback_timestamp)
}
...
fn parse_gemini_headless_jsonl(
    path: &Path,
    source: &str,
    fallback_timestamp: i64,
) -> Vec<UnifiedMessage> {
...
fn parse_gemini_headless_value(
    ...
    build_messages_from_stats(stats, model_hint, session_id, source, timestamp)
}
...
mod tests {
    ...
    #[test]
    fn test_parse_gemini_structure_with_array_content() {
        // Post-Feb-7 2026 format: user messages use content: [{text: "..."}] array
        let json = r#"{
            "sessionId": "ses_456",
            "projectHash": "def456",
            "startTime": "2026-02-10T09:00:00Z",
            "lastUpdated": "2026-02-10T09:05:00Z",
            "messages": [
                {
                    "id": "msg_1",
        ...
        // Full parse test: should produce one message (only "gemini" type)
        let messages = parse_gemini_session(session, "gemini", 0);
    ...
    #[test]
    fn test_parse_antigravity_file_uses_antigravity_source() {
        let json = r#"{
            "sessionId": "ses_ag_789",
            "projectHash": "ghi789",
            ...
            "messages": [
                {
                    ...
                    "type": "user",
                    "content": [{"text": "Write me a function"}]
                },
                {
                    "id": "msg_2",
        ...
        let messages = parse_antigravity_file(file.path());
        assert_eq!(messages.len(), 1);
        assert_eq!(messages[0].client, "antigravity");
        assert_eq!(messages[0].model_id, "gemini-3-pro");
